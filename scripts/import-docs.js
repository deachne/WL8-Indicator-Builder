...
/**
 * Import WL8 Documentation Script
 * 
 ...
 * 
 * Usage:
 *   node scripts/import-docs.js [--use-chroma]
 */
...
// Configuration
const GITHUB_REPO = 'https://github.com/deachne/WL8-pkm';
...
async function cloneOrUpdateRepo() {
  ...
  if (fs.existsSync(repoDir)) {
    ...
    try {
      execSync(`git clone ${GITHUB_REPO} ${repoDir}`, { stdio: 'inherit' });
...
async function processMarkdownFiles(repoDir) {
  ...
  const apiDocsDir = path.join(repoDir, 'API');
  ...
  const frameworkDocsDir = path.join(repoDir, 'Framework');
